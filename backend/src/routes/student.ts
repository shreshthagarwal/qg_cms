...
router.patch('/:id', authenticate, authorizeAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    if (req.body.firstname !== undefined) userUpdates.firstname = req.body.firstname;
    if (req.body.lastname !== undefined) userUpdates.lastname = req.body.lastname;
...
router.post('/:id/reset-password', authenticate, authorizeAdmin, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const db = getDb();
