        ...
        db = {
            ...
            async findAllStudentsWithProfiles() {
                ...
                return students.map(student => ({
                    ...student,
                    student_profiles: profiles.find(profile => (profile.user_id === student.id) || (profile.userid === student.id)) || null
                }));
            },
            // Leads
            ...
            async findStudentLmsContent(studentId) {
            ...
            // ==================== LMS METHODS ====================
            async createLmsContent(contentData) {
                const { data, error } = await client
                    .from('lms_content')
                    .insert({
                    id: contentData.id,
            ...
            async deleteLmsContent(contentId) {
            ...
            async createLmsAssignment(assignmentData) {
            ...
            async getStudentLmsAssignments(studentId) {
            ...
            async getLmsAssignmentById(assignmentId) {
                const { data, error } = await client
                    .from('lms_assignments')
                    .select('*')
                    .eq('id', assignmentId)
            ...
            async updateLmsAssignmentStatus(assignmentId, status, notes) {
                const updateData = {
                    status,
                    updated_at: new Date().toISOString()
                };
            ...
            async createLmsTimeline(timelineData) {
                const { data, error } = await client
                    .from('lms_timeline')
                    .insert({
                    id: timelineData.id,
            ...
            async getStudentLmsTimeline(studentId) {
          ...
          `)
            ...
            // Evaluations
            async updateLmsContentScore(id, score) {
                const { data, error } = await client
                    .from('lms_content')
            ...
            async assignQuizToStudents(quizId, studentIds, deadline, duration) {
            ...
            async deleteQuizSubmission(submissionId) {
                const { error } = await client
          ...
          `)
            ...
            async getQuizSubmissionById(submissionId) {
                const { data, error } = await client
                    .from('quiz_submissions')
                    .select(`
            *,
          ...
          `)
            ...
            async releaseQuizScores(quizId) {
                const { data, error } = await client
                    .from('quiz_submissions')
            ...
            async findActiveQuizzesForStudent(studentId) {
          ...
          `)
                ...
                const activeQuizzes = assignments
                    ?.filter((assignment) => !attemptedQuizIds.has(assignment.quiz_id))
                    .map((assignment) => ({
            ...
            async findAttemptedQuizzesForStudent(studentId) {
                const { data, error } = await client
                    .from('quiz_submissions')
                    .select(`
            *,
          ...
          `)
            ...
            async checkQuizAccess(studentId, quizId) {
            ...
            async hasStudentAttemptedQuiz(studentId, quizId) {
                const { data, error } = await client
                    .from('quiz_submissions')
                    .select('*')
                    .eq('student_id', studentId)
            ...
            async submitQuizAttempt(submissionData) {
          ...
          `)
            ...
            async assignAssignmentToStudents(assignmentId, studentIds) {
                const assignments = studentIds.map(studentId => ({
            ...
            async removeStudentFromAssignment(assignmentId, studentId) {
                const { error } = await client
                    .from('assignment_assignments')
                    .delete()
            ...
            async getAssignmentAssignedStudents(assignmentId) {
                const { data, error } = await client
                    .from('assignment_assignments')
                    .select('*')
                    .eq('assignment_id', assignmentId);
            ...
            // ==================== TRADING METHODS ====================
            // Strategies
            async getStudentStrategies(studentId) {
                const { data, error } = await client
            ...
            async getStrategyById(strategyId, studentId) {
                const { data, error } = await client
                    .from('trading_strategies')
                    .select('*')
                    .eq('id', strategyId)
            ...
            async updateStrategy(strategyId, studentId, updateData) {
                const { data, error } = await client
                    .from('trading_strategies')
                    .update(updateData)
                    .eq('id', strategyId)
            ...
            async deleteStrategy(strategyId, studentId) {
                const { error } = await client
                    .from('trading_strategies')
                    .delete()
                    .eq('id', strategyId)
            ...
            async getBacktestById(backtestId, studentId) {
                const { data, error } = await client
                    .from('strategy_backtests')
                    .select('*')
                    .eq('id', backtestId)
            ...
            async getStrategyBacktests(strategyId, studentId) {
                const { data, error } = await client
                    .from('strategy_backtests')
                    .select('*')
            ...
            async updateBacktestStatus(backtestId, status, errorMessage) {
            ...
            async completeBacktest(backtestId, results) {
                const { error } = await client
                    .from('strategy_backtests')
                    .update({
                    status: 'completed',
            ...
            async getOpenPosition(studentId, symbol) {
                const { data, error } = await client
                    .from('paper_trades')
                    .select('*')
                    .eq('student_id', studentId)
            ...
            async closeTrade(tradeId, exitPrice, pnl, pnlPercent) {
                const { data, error } = await client
                    .from('paper_trades')
                    .update({
            ...
            async getStudentTrades(studentId) {
                const { data, error } = await client
                    .from('paper_trades')
                    .select('*')
                    .eq('student_id', studentId)
            ...
            async getOpenPositions(studentId) {
                const { data, error } = await client
                    .from('paper_trades')
                    .select('*')
                    .eq('student_id', studentId)
          ...
          `)
            ...
            async getAllDailyPnLReports(date) {
                let query = client
          ...
          `);
            ...
            async reviewDailyPnL(reportId, adminId, notes) {
                const { error } = await client
                    .from('daily_pnl_reports')
                    .update({
                    reviewed_by: adminId,
            ...
            async createTradingReport(reportData) {
                const { data, error } = await client
            ...
            async getStudentTradingReports(studentId) {
                const { data, error } = await client
                    .from('trading_reports')
                    .select('*')
                    .eq('student_id', studentId)
            ...
            async getTradingReportById(reportId, studentId) {
                const { data, error } = await client
                    .from('trading_reports')
                    .select('*')
                    .eq('id', reportId)
            ...
            async updateTradingReport(reportId, studentId, updateData) {
                const { data, error } = await client
                    .from('trading_reports')
                    .update({
                    ...updateData,
            ...
            async deleteTradingReport(reportId, studentId) {
                const { error } = await client
                    .from('trading_reports')
                    .delete()
                    .eq('id', reportId)
            ...
            async reviewTradingReport(reportId, adminId, status, notes) {
                const { error } = await client
                    .from('trading_reports')
                    .update({
                    status,
            ...
            // Attendance methods
            async findAttendanceByDateRange(startDate, endDate) {
                const { data, error } = await client
                    .from('attendance')
                    .select('*')
